...
/* Delivery address:
   1) try many shapes on the ORDER itself
   2) if missing, fall back to the GUEST's saved address */
const readDeliveryAddress = (o, guest) => {
    const raw =
        o?.delivery_address ??
        o?.address ??
        o?.shipping_address ??
    ...
    // → build a pretty line from the guest cache
    if (guest?.address) {
        const { line1, line2, city, state, postal_code } = guest.address;
...
export default function AllOrders() {
    ...
    /* 👥 Guests cache: { [guest_id]: { id, name, phone, address:{...} } } */
    ...
    const [expanded, setExpanded] = useState(() => new Set()); // which cards are open

    /* 🔴 Live: realtime, chime, flash */
    ...
    /* ✨ Make a card flash for a few seconds */
    ...
    const upsertGuests = (rows = []) =>
        setGuests((prev) => {
            ...
            for (const g of rows) {
                ...
                const postal = g.postal_code != null ? String(g.postal_code) : '';
                ...
                next[g.id] = {
                    ...
                    phone: phone || '',
                    address: {
                        line1: g.address_line1 || '',
                        line2: g.address_line2 || '',
                        city: g.city || '',
                        state: g.state || '',
                        postal_code: postal,
                    },
    ...
    const fetchOrders = async (opts = { setBusy: true }) => {
        try {
            ...
            setOrders(arr);
            setErr(null);
            // also load related guests so name/phone/address can show
    ...
    /* ------------------------------------------------------------------------ */
    useEffect(() => {
        // Orders changes (insert/update/delete)
        ...
        // If a guest changes name/phone/address, refresh our cache and softly flash related orders
    ...
    return (
      ...
      `}</style>
            ...
            <div className="flex flex-wrap items-center gap-2">
                <span
                    className={`inline-flex items-center gap-2 rounded-xl border px-3 py-1.5 text-sm ${connected
            ...
            {/* Cards: masonry via CSS columns; each card has break-inside: avoid */}
            {/* Cards: simple CSS grid (no masonry) */}
            {!loading && !err && filtered.length > 0 && (
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
                    {filtered.map((order) => {
                        ...
                        const guest = guests[order?.guest_id];
                        const name = (guest?.name || '').trim();
                        const phone = guest?.phone || '';
                        const flashing = flashIds.has(order.id);

                        const isDelivery = /delivery/.test(fulfillmentRaw);
                        const address = isDelivery ? readDeliveryAddress(order, guest) : '';

                        return (
                            <div
                                key={order.id}
                                className={`rounded-2xl bg-white p-4 shadow-sm border transition-shadow ${flashing ? 'border-amber-400 flash-ring' : 'border-slate-200'
                                    }`}
                            >
                                {/* ------------------------------------------------------------------ */}
                                {/* Header: big Name + Phone, then order status + fulfillment          */}
                                {/* ------------------------------------------------------------------ */}
                                <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0">
                                        ...
                                        {/* order #, payment status, fulfillment */}
